...
/* ############### ARDUINO NANO 33 BLE / NANO RP2040 CONNECT ############### */

#if defined(CONFIG_BOARD_ARDUINO_NANO_33_BLE) || defined(CONFIG_BOARD_ARDUINO_NANO_CONNECT)
...
/* the PDM mic zephyr device — first phandle in zephyr,user 'pdms' */
static const struct device *const dmic_dev =
	DEVICE_DT_GET(DT_PHANDLE_BY_IDX(DT_PATH(zephyr_user), pdms, 0));
...
int pdm_read(void **buffer, size_t *size) {
	...
	/* Non-blocking: PDM.available() polls. */
	return dmic_read(dmic_dev, 0, buffer, size, 0);
#else
...
void pdm_gain(int gain) {
	...
	NRF_PDM->GAINL = gain;
#else
	/* RP2040 driver applies gain via DT 'gain' property (compile-time);
	 * runtime change is not supported here. */
	(void)gain;
#endif
}
#endif
